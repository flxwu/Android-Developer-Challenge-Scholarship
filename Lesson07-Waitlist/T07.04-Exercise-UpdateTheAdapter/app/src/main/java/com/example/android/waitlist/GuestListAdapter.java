...
public class GuestListAdapter extends RecyclerView.Adapter<GuestListAdapter.GuestViewHolder> {
    ...
    public void onBindViewHolder(GuestViewHolder holder, int position) {
        ...
        String GUEST_NAME = mCursor.getString(mCursor.getColumnIndex(WaitlistContract.WaitlistEntry.COLUMN_GUEST_NAME));
        // COMPLETED (7) Call getInt on the cursor to get the party size
        String PARTY_SIZE = String.valueOf(mCursor.getInt(mCursor.getColumnIndex(WaitlistContract.WaitlistEntry.COLUMN_PARTY_SIZE)));
        // COMPLETED (8) Set the holder's nameTextView text to the guest's name
        holder.nameTextView.setText(GUEST_NAME);
        // COMPLETED (9) Set the holder's partySizeTextView text to the party size
        holder.partySizeTextView.setText(PARTY_SIZE);
    }
